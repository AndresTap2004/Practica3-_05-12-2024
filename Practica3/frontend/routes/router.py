...
# Ruta para ordenar generadores
@router.post('/generadores/ordenar')
def ordenar_generadores():
    data = request.get_json()
    url = f"http://localhost:8086/api/generador/sort/{data.get('metodo')}/{data.get('atributo')}/{data.get('orden')}"
    response = requests.get(url)
    if response.ok:
        sorted_data = response.json().get("data")
        return jsonify(sorted_data)
    else:
        return jsonify({"error": "No se pudo ordenar los generadores."}), 500

# Ruta para buscar generadores
@router.post('/generadores/buscar')
def buscar_generadores():
    data = request.get_json()

    url = f"http://localhost:8086/api/generador/search/{data.get('searchAttr')}/{data.get('searchField')}"
    response = requests.get(url)
    if response.ok:
        search_results = response.json().get("data")
        return jsonify(search_results)
    else:
        return jsonify({"error": "No se pudo realizar la búsqueda de generadores."}), 500

# Ruta para limpiar la lista de generadores
@router.get('/generadores/limpiar')
def limpiar_generadores():
    url = "http://localhost:8086/api/generador/list"
    response = requests.get(url)

    if response.status_code == 200:
        data = response.json()
        generadores = data.get("data", [])
        return jsonify(generadores)
    else:
        return jsonify({"error": "Error al obtener la lista de generadores."}), response.status_code
